...
from agents.security import bash_security_hook
...
def create_minimal_client(
    ...
    model: str,
    verbose: bool = True,
    http_mcp_servers: dict[str, dict] | None = None,
    allowed_tools: list[str] | None = None,
) -> ClaudeSDKClient:
    """
    Create a minimal Claude SDK client for swarm agents.

    Only includes HTTP-based MCP servers (safe for concurrent access).
    Excludes stdio MCP servers (like Puppeteer) which cause timeout errors
    when multiple agents try to spawn processes concurrently.

    Args:
        project_dir: Directory for the project
        model: Claude model to use
        verbose: If True, print status messages
        http_mcp_servers: Optional dict of HTTP/SSE MCP servers to include.
            Only servers with type="http" or type="sse" are safe for concurrent use.
            Example: {"archon": {"type": "http", "url": "http://localhost:8000/mcp"}}
        allowed_tools: Optional list of additional tool names to allow (for MCP tools)

    Returns:
        Configured ClaudeSDKClient with only HTTP-based MCP servers
    """
    ...
    # Build MCP servers dict - only HTTP-based servers (safe for concurrent access)
    mcp_servers = {}
    if http_mcp_servers:
        for name, config in http_mcp_servers.items():
            server_type = config.get("type", "")
            if server_type in ("http", "sse"):
                mcp_servers[name] = config
            elif verbose:
                print(f"   [WARNING] Skipping stdio MCP server '{name}' - not safe for concurrent swarm agents")
    ...
    return ClaudeSDKClient(
        options=ClaudeCodeOptions(
            model=model,
            system_prompt="You are an expert software architect and product analyst. Focus on thorough analysis and clear documentation.",
            allowed_tools=tools,
