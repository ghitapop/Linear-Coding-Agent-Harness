...
from server.harness_agent.orchestrator.project_registry import create_project_registry
...
async def run_cli_mode(args: argparse.Namespace) -> int:
    ...
    Returns:
        Exit code.
    """
    from server.harness_agent.adapters.cli_adapter import CLIAdapter, create_cli_adapter
    from server.config.loader import load_config, create_default_config
    from server.harness_agent.orchestrator.phase_runner import create_default_runner
    from server.harness_agent.orchestrator.shutdown import create_shutdown_handler
    from server.harness_agent.orchestrator.heartbeat import create_heartbeat_manager
    ...
    try:
        while True:
                ...
                try:
                    ...
                    if result == "__NEW_PROJECT__":
                        ...
                        if project_dir_path:
                            ...
                            # Pass swarm config if available
                            swarm_cfg = config.swarm.model_dump() if config and hasattr(config, 'swarm') else None
                            current_runner = create_default_runner(
                                project_dir_path,
                                include_planning_phases=True,
            ...
            if current_project_dir and current_runner:
                ...
                # Pass swarm config if available (for HTTP MCP servers like Archon)
                swarm_cfg = config.swarm.model_dump() if config and hasattr(config, 'swarm') else None
            ...
            try:
                async with heartbeat:
                    ...
                    async def approval_callback(summary: str, phase: str) -> tuple[bool, Optional[str]]:
...
async def run_api_mode(args: argparse.Namespace) -> int:
    ...
    """
    import uvicorn
    from server.api_routes.main import create_app
...
async def show_status(args: argparse.Namespace) -> int:
    ...
    from server.harness_agent.orchestrator.resume import ResumeManager
    ...
    if args.project_dir:
        ...
        project_dir = Path(os.getenv("WORKSPACE_PATH", "./workspaces"))
...
async def resume_project(args: argparse.Namespace) -> int:
    ...
    from server.harness_agent.adapters.cli_adapter import create_cli_adapter
...
async def stop_project(args: argparse.Namespace) -> int:
    ...
    """
    from server.config.loader import load_config
    from server.harness_agent.orchestrator.state_machine import StateMachine, PipelineStatus
